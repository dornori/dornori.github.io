...
(function () {
    ...
    if (typeof CONFIG !== 'undefined') {
        ...
        var useSql = CONFIG.dataSource === 'sql' && CONFIG.endpoints && CONFIG.endpoints.api;
        CONFIG.data.shippingJson     = useSql ? CONFIG.endpoints.api + '/shipping'  : sitBase + 'data/shipping.json';
        CONFIG.data.countriesJson    = useSql ? CONFIG.endpoints.api + '/countries' : sitBase + 'data/countries.json';
        CONFIG.data.productsJson     = useSql ? CONFIG.endpoints.api + '/products'  : sitBase + 'data/products.json';
        CONFIG.data.reviewsJson      = useSql ? CONFIG.endpoints.api + '/reviews'   : sitBase + 'data/reviews.json';
    ...
    function _cartUrl(lang) {
        // STANDALONE MODULE: flat cart.html instead of the full site's
        // lang-prefixed /en/cart/ folder route.
        return sitBase + 'cart.html';
    }
    ...
    function _patchProductLinks() {
        // STANDALONE MODULE: no-op. The full site used this to rewrite
        // "product.html?id=" links into lang-prefixed "/en/product/?id="
        // folder routes. This flat module's links are already correct
        // ("product.html?id=...") straight out of shop.js, so there's
        // nothing to patch.
    }
