...
var Shop = (() => {
   ...
   <span class="webshop-cart-icon__badge" aria-live="polite">0</span>`;
    ...
    function renderDropdown() {
      loadLang().then(() => {
        ...
        // STANDALONE MODULE: flat URL, no lang/slug folder routing.
        dropdown.innerHTML = `
  ...
  function buildProductCard(p, options = {}) {
    ...
    // STANDALONE MODULE: flat single-page URL instead of the full site's
    // lang-prefixed /en/product/ folder route (no folder routing here).
    const prodUrl = `product.html?id=${p.id}`;
    const hasUrl  = !!p.id;
  ...
  function renderBuyNowButton(divId, productId, options = {}) {
    ...
    loadLang().then(() => {
      loadProducts().then(() => {
        ...
        // STANDALONE MODULE: flat URL, no lang/slug folder routing.
        const prodUrl = `product.html?id=${product.id}`;
