...
async function loadFormData() {
  ...
  try {
    ...
    const langFile = `${base}${langDir}${lang}/form.json`;
    const baseFile = `${base}${langDir}en/form.json`;
    let response;
    try {
...
window.selectAnswer = function (optionIndex) {
  ...
  if (selected.resolves === true) { showResolvedAndRedirect(); return; }
  if (selected.next === 'resolved_end') { showResolvedAndRedirect(); return; }
  ...
  if (selected.next === "contact") {
    ...
    if (nextIndex !== -1) { currentQuestionIndex = nextIndex; renderCurrentQuestion(); }
    else { currentQuestionIndex++; renderCurrentQuestion(); }
  } else {
...
function showResolvedAndRedirect() {
  const redirectUrl = currentData?.endpoints?.resolved_end?.redirect_url || SITE_CONFIG.defaults.redirectUrl;
  const delay       = currentData?.endpoints?.resolved_end?.delay_seconds || 2;
  const message     = currentData?.endpoints?.resolved_end?.message       || SITE_CONFIG.defaults.redirectMessage;

  showToast(message, 1500);
  setTimeout(() => {
    let fullUrl = redirectUrl;
    if (redirectUrl.startsWith('/')) fullUrl = window.location.origin + redirectUrl;
    ...
    if (response.ok) {
      ...
      setTimeout(() => {
        if (typeof window.showResolvedModal === 'function') {
          ...
          window.location.href = window.location.origin + SITE_CONFIG.defaults.redirectUrl;
        }
      }, 2000);
    } else {
