...
echo ========================================
echo.
echo All configuration will be asked at the start.
echo.

:: ============================================
:: GATHER ALL INPUTS AT THE BEGINNING
:: ============================================
...
echo.
echo Where should your D1 database be hosted?
echo 1. Global (default - best performance for most users)
echo 2. EU only (recommended for GDPR compliance)
echo 3. FedRAMP (for US government / compliance requirements)
echo.
set /p JURISDICTION_CHOICE="Choose (1-3, default 1): "
if "%JURISDICTION_CHOICE%"=="2" (
    set "JURISDICTION=eu"
    echo [Selected] EU jurisdiction
) else if "%JURISDICTION_CHOICE%"=="3" (
    set "JURISDICTION=fedramp"
    echo [Selected] FedRAMP jurisdiction
) else (
    set "JURISDICTION="
    echo [Selected] Global jurisdiction
)
...
:: Create D1 Database with jurisdiction
if "%JURISDICTION%"=="" (
    curl -s -X POST "https://api.cloudflare.com/client/v4/accounts/%ACCOUNT_ID%/d1/database" ^
      -H "Authorization: Bearer %API_TOKEN%" ^
      -H "Content-Type: application/json" ^
      -d "{\"name\":\"%WORKER_NAME%_db\"}" > "%TEMP%\db.json"
) else (
    curl -s -X POST "https://api.cloudflare.com/client/v4/accounts/%ACCOUNT_ID%/d1/database" ^
      -H "Authorization: Bearer %API_TOKEN%" ^
      -H "Content-Type: application/json" ^
      -d "{\"name\":\"%WORKER_NAME%_db\", \"jurisdiction\":\"%JURISDICTION%\"}" > "%TEMP%\db.json"
)
...
:: ============================================
:: ENABLE SUBDOMAIN + CORS + RATE LIMIT
:: ============================================
...
:: ============================================
:: CREATE INITIAL ADMIN USER (with fixed hashing)
:: ============================================
echo.
echo ========================================
...
if errorlevel 1 (
    ...
    echo Role: admin
    echo Please save your admin password!
    echo ========================================
)
...
if /i "%CONFIG_EMAIL%"=="Y" (
    echo.
    echo Configuring email credentials...
    :: (Add your email config logic here if needed)
)
