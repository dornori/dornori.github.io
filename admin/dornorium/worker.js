  ...
  if (path === "/api/admin/debug-token" && method === "GET") {
    ...
    return json({ 
      ...
      if (path === "/api/admin/email-addresses" && method === "POST") {
        ...
        try {
          await addEmailAddress(env, newEmail, label, await validateCategory(action || label, env), language, address_type || 'incoming');
          await bumpConfigVersion(env);
          return json({ success: true });
        } catch (e) {
          return json({ error: e.message }, 400);
    ...
    return json({ success: false, error: e.message }, 400);
      ...
      if (path === "/api/admin/setup/domain-remove" && method === "DELETE") {
        ...
        try {
          ...
          // Delete email_addresses for this domain
          if (domainName) {
            await env.DB.prepare(
              "DELETE FROM email_addresses WHERE lower(email) LIKE ?"
