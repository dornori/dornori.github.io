...
async function getEmailAddressConfig(env, toAddress) {
  ...
  try {
    const row = await env.DB.prepare("SELECT action, language FROM incoming_email_addresses WHERE is_active = 1 AND lower(email) = ?").bind(clean).first();
    if (row && row.action) {
  ...
  if (path === "/api/admin/debug-token" && method === "GET") {
    ...
    return json({ 
      ...
      if (path === "/api/admin/email-addresses/by-email" && method === "PUT") {
        ...
        try {
          await env.DB.prepare(
            "UPDATE incoming_email_addresses SET label=?, action=?, language=? WHERE lower(email)=lower(?)"
          ).bind(label, action || label, language, targetEmail).run();
          await bumpConfigVersion(env);
          return json({ success: true });
        } catch (e) {
          return json({ error: e.message }, 400);
      ...
      // ── Incoming email address mappings (separate table) ──
      if (path === "/api/admin/incoming-email-addresses" && method === "GET") {
        const token = (request.headers.get("Authorization") || "").replace("Bearer ", "");
        const email = await verifyToken(token, env);
        if (!email) return json({ error: "Unauthorized" }, 401);
        if (!await checkAccess(email, "settings", "view", env)) return json({ error: "Permission denied" }, 403);
        const r = await env.DB.prepare("SELECT * FROM incoming_email_addresses ORDER BY email").all();
        return json({ success: true, addresses: r.results || [] });
      }

      if (path === "/api/admin/incoming-email-addresses" && method === "POST") {
        const token = (request.headers.get("Authorization") || "").replace("Bearer ", "");
        const email = await verifyToken(token, env);
        if (!email) return json({ error: "Unauthorized" }, 401);
        if (!await checkAccess(email, "settings", "create", env)) return json({ error: "Permission denied" }, 403);
        ...
        try {
          await env.DB.prepare(
            "INSERT OR REPLACE INTO incoming_email_addresses (email, label, action, language) VALUES (?, ?, ?, ?)"
          ).bind(targetEmail, label || '', action || label || '', language || 'en').run();
          await bumpConfigVersion(env);
          return json({ success: true });
        } catch (e) {
          return json({ error: e.message }, 400);
      ...
      if (path.match(/^\/api\/admin\/incoming-email-addresses\/\d+$/) && method === "DELETE") {
        const token = (request.headers.get("Authorization") || "").replace("Bearer ", "");
        const email = await verifyToken(token, env);
        if (!email) return json({ error: "Unauthorized" }, 401);
        if (!await checkAccess(email, "settings", "delete", env)) return json({ error: "Permission denied" }, 403);
        const id = parseInt(path.split("/")[5]);
        await env.DB.prepare("DELETE FROM incoming_email_addresses WHERE id = ?").bind(id).run();
        await bumpConfigVersion(env);
        return json({ success: true });
      }
    ...
    return json({ success: false, error: e.message }, 400);
      ...
      if (path === "/api/admin/setup/cloudflare-zone" && method === "POST") {
        ...
        try {
          ...
          // Store domain in our tracking table
          try {
            await env.DB.prepare(
              "INSERT OR REPLACE INTO cloudflare_domains (zone_id, domain_name, enabled, status) VALUES (?, ?, 0, 'pending_setup')"
            ).bind(zone.id, domain).run();
          } catch (e) {
            console.warn("Failed to track domain in DB:", e.message);
          }
      ...
      if (path === "/api/admin/setup/email-routing" && method === "POST") {
        ...
        try {
          ...
          // Mark domain as enabled in our DB
          try {
            await env.DB.prepare(
              "UPDATE cloudflare_domains SET enabled = 1, status = 'active', updated_at = CURRENT_TIMESTAMP WHERE zone_id = ?"
            ).bind(zoneId).run();
          } catch (e) {
            console.warn("Failed to update domain status:", e.message);
          }
      ...
      if (path === "/api/admin/setup/email-routing-disable" && method === "POST") {
        ...
        try {
          await callCloudflareAPI("POST", `/zones/${zoneId}/email/routing/disable`, null, env);
          
          // Mark as disabled in DB
          await env.DB.prepare(
            "UPDATE cloudflare_domains SET enabled = 0, updated_at = CURRENT_TIMESTAMP WHERE zone_id = ?"
          ).bind(zoneId).run();
      ...
      // Remove domain completely (from CF and DB)
      if (path === "/api/admin/setup/domain-remove" && method === "DELETE") {
        const token = (request.headers.get("Authorization") || "").replace("Bearer ", "");
        const email = await verifyToken(token, env);
        if (!email) return json({ error: "Unauthorized" }, 401);
        if (!await checkAccess(email, "settings", "edit", env)) return json({ error: "Permission denied" }, 403);
        ...
        try {
          ...
          // DELETE EMAILS FIRST — exact match on full domain, both tables
          let emailsDeleted = 0;
          const exactDomainLower = domainName.toLowerCase();
          const pattern = `%@${exactDomainLower}`;

          for (const table of ['incoming_email_addresses', 'email_addresses']) {
            const toDeleteResult = await env.DB.prepare(
              `SELECT id, email FROM ${table} WHERE lower(email) LIKE ?`
            ).bind(pattern).all();
            const confirmed = (toDeleteResult.results || []).filter(row => {
              return row.email.toLowerCase().split('@')[1] === exactDomainLower;
            });
...
if (path.startsWith("/api/admin/setup/routing-rule/") && method === "PUT") {
  ...
  async email(message, env) {
    try {
      ...
      // Only process mail for enabled domains
      if (message.to) {
        const toDomain = message.to.split("@")[1]?.toLowerCase() || "";
        const dbDomain = await env.DB.prepare(
          "SELECT enabled FROM cloudflare_domains WHERE domain_name = ? AND enabled = 1"
        ).bind(toDomain).first();
        
        if (!dbDomain) {
          console.log(`Ignoring mail to ${message.to} (domain not enabled or not found)`);
          return;
        }
      }
