/**
 * shop-loader.js (v3 - with feature flags support)
 */
...
export async function mountShopEmbeds(container) {
  ...
  if (cartSlot && !cartSlot.querySelector('.webshop-cart-icon-wrap')) {
  ...
  if (currSlot && !currSlot.querySelector('select, .webshop-currency-selector')) {
  ...
  for (const el of shopProductEls) {
    ...
    // Your existing Shop.buildProductCard already supports options!
  ...
  if (miniCart && miniCart.id && !miniCart.hasAttribute('data-initialized')) {
    Shop.renderMiniCart(miniCart.id);
    miniCart.setAttribute('data-initialized', 'true');
  }
}