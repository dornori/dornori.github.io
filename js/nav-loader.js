 ...
 * nav-loader.js
 * ─────────────────────────────────────────────────────────────────────────────
 * Reads labels from window.T (loaded by i18n.js from lang/[code]/common.json).
 * Exposes window.renderNav() so setLang() can re-render without a page reload.
 * Uses <a href> tags so Google can crawl all pages.
 * Generates language-specific pretty URLs via url_slugs in the lang bundle.
...
window.renderNav = () => {
    ...
    // Update settings tab label on re-render (language switch)
    ...
    // Update topBar label text nodes on re-render
    ...
    if (desktopNav) {
        ...
        SITE_CONFIG.navigation.forEach(item => {
            ...
            setSVGContent(iconEl, FALLBACK_SVG);  // XSS-safe (FIX #5)
