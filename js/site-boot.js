...
(function () {
    ...
    // ── Load languages manifest (NO HARDCODING) ────────────────────────────────
    // This must load EARLY so all modules can use it without hardcoded fallbacks.
    // The manifest is generated by scripts/generate-languages-manifest.js
    (function loadLanguagesManifest() {
        var manifestUrl = BASE_PATH + 'data/languages.json';
        fetch(manifestUrl)
            .then(function(r) {
            ...
            .then(function(manifest) {
                // Cache manifest globally
                window.__languagesManifest = manifest;
                window.__supportedLanguages = manifest.languages || [];
            })
            .catch(function(err) {
                console.warn('[site-boot] Failed to load languages manifest:', err);
                // Graceful fallback: empty array, will use config.defaultLanguage
