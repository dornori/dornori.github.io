...
import { createFromHTML, setTextContent } from '../utils/dom-safe.js';
...
const Payment = (() => {
  ...
  const _paypal = {
    ...
    async render(cart, totals, orderRef, el) {
      if (!window.paypal) {
        el.innerHTML = '';
        const errorEl = createFromHTML(`<div style="padding:20px;text-align:center;border:1px dashed var(--c-border);border-radius:var(--radius);color:var(--c-text-3);font-size:0.85rem;line-height:1.6;">
  ...
  const _stripe = {
    ...
    async render(cart, totals, orderRef, el) {
      ...
      if (!cfg.intentEndpoint) {
        el.innerHTML = '';
        const errorEl = createFromHTML(`<div style="padding:20px;text-align:center;color:var(--c-text-3);font-size:0.85rem;border:1px dashed var(--c-border);border-radius:var(--radius);">
